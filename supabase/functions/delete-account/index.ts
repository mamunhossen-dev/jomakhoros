...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use service role to delete the user (cascades via FKs / triggers, plus we clean profile-scoped tables)
    const admin = createClient(supabaseUrl, serviceKey);

    // Best-effort cleanup of user data (RLS bypass with service role)
    const tables = [
