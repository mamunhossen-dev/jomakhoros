...
export default function Login() {
  ...
  const { enabled: registrationEnabled } = useFeatureFlag('user_registration', { defaultEnabled: true, label: 'নতুন রেজিস্ট্রেশন', category: 'auth' });
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) {
      toast({ title: 'Google সাইন-ইন ব্যর্থ', description: result.error.message, variant: 'destructive' });
      return;
    }
    ...
    <div className="flex min-h-screen">
      <div className="hidden lg:flex lg:w-2/5 xl:w-1/3 flex-col justify-between bg-sidebar p-10">
        ...
        <div>
          <h1 className="font-display text-4xl font-bold leading-tight text-sidebar-foreground whitespace-pre-line">
            {fmt(a.login_side_title, brand.name)}
          </h1>
          <p className="mt-4 text-lg text-sidebar-foreground/60">
            {fmt(a.login_side_subtitle, brand.name)}
          </p>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          <div>
            <h2 className="font-display text-2xl font-bold">{a.login_heading}</h2>
            <p className="mt-1 text-muted-foreground">{a.login_subheading}</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">{a.login_email_label}</Label>
            ...
            <Button type="submit" className="w-full" disabled={loading}>
              {loading ? a.login_submit_loading : a.login_submit_label}
            </Button>
          </form>
          ...
          {registrationEnabled && (
            <p className="text-center text-sm text-muted-foreground">
              {a.login_no_account}{' '}
              <Link to="/register" className="font-medium text-primary hover:underline">{a.login_register_link}</Link>
            </p>
          )}
        </div>
      </div>
